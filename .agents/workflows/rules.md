...
5. **Ask before using the browser.** Never use the `browser_subagent` or any browser-based testing tools without explicit, per-task permission from the user. Permission granted for one audit does not carry over to subsequent tasks or fixes.
...
- ❌ Using the browser for verification/testing without explicit, per-task approval
